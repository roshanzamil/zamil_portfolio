      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid gap-8 md:grid-cols-1 lg:grid-cols-2">
          {experiences.map((experience) => (
            <Card key={experience.company} className="flex flex-col overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-xl">
              {experience.image && (
              ...
              <CardContent className="flex-1 p-6 space-y-3">
                <CardTitle className="text-xl font-bold">{experience.role} at {experience.company}</CardTitle>
                <p className="text-sm font-medium text-muted-foreground">{experience.period}</p>
                <CardDescription className="text-sm text-foreground/80">{experience.description}</CardDescription>
              ...
              <CardFooter className="p-6 pt-0 flex justify-end gap-2">
                <Button variant="outline" size="sm" asChild>
                  <Link href={experience.liveUrl} target="_blank">
